console.log("WelCome To My Spotify....!");
// Initialize the variables
let songIndex = 0;
let audioElement = new Audio("./songs/2.mp3");
let masterPlay = document.getElementById("masterPlay");
let progressBar = document.getElementById("myProgressBar");
let gifImg = document.getElementById("gif");
let masterSongName = document.getElementById('masterSongName');
let songItems = Array.from(document.getElementsByClassName('songItem'));
let changeGif = document.getElementById("gif");


// audioElement.play()



const songs = [
    {
        songName: "Na Jaana - Salam-e-Ishq", 
        filePath:"./songs/1.mp3",
        coverPath:"./Cover/1.jpg",
        songDuration:1
    },
    {
        songName: "Tumhari Kasam - Salam-e-Ishq", 
        filePath:"./songs/2.mp3",
        coverPath:"./Cover/2.jpg"
    },
    {
        songName: "Bhula Dena - Salam-e-Ishq", 
        filePath:"./songs/3.mp3",
        coverPath:"./Cover/3.jpg"
    },
    {
        songName: "Sakhiyaan - Salam-e-Ishq", 
        filePath:"./songs/4.mp3",
        coverPath:"./Cover/4.jpg"
    },
    {
        songName: "Rabba - Salam-e-Ishq", 
        filePath:"./songs/5.mp3",
        coverPath:"./Cover/5.jpg"
    },
    {
        songName: "Janji-Heroes-Tonight-feat-Johnning-NCS-Release", 
        filePath:"./songs/6.mp3",
        coverPath:"./Cover/6.jpg"
    },
    {
        songName: "Different Heaven & EH!DE - My Heart [NCS Release]", 
        filePath:"./songs/7.mp3",
        coverPath:"./Cover/7.jpg"
    },
    {
        songName: "DEAF KEV - Invincible[NCS Release]-320k", 
        filePath:"./songs/8.mp3",
        coverPath:"./Cover/8.jpg"
    },
    {
        songName: "Cielo - Huma- Huma", 
        filePath:"./songs/9.mp3",
        coverPath:"./Cover/9.jpg"
    },
    {
        songName: "Warriyo - Mortals[NCS Release]", 
        filePath:"./songs/10.mp3",
        coverPath:"./Cover/10.jpg"
    }
]
songItems.forEach((element, i) =>{
    element.getElementsByTagName("img")[0].src = songs[i].coverPath;
    element.getElementsByClassName("songName")[0].innerText = songs[i].songName;
})


masterPlay.addEventListener("click",() => {
    if(audioElement.paused || audioElement.currentTime<=0){
        audioElement.play();
        masterPlay.classList.remove("fa-circle-play");
        masterPlay.classList.add("fa-circle-pause");
        changeGif.style.opacity = 1;
    }
    else{
        audioElement.pause();
        masterPlay.classList.add("fa-circle-play");
        masterPlay.classList.remove("fa-circle-pause");
        changeGif.classList.remove("img");
        changeGif.style.opacity = 0;
    }

});

audioElement.addEventListener("timeupdate", () => {
    console.log("TimeUpdate")
    let progress = parseInt((audioElement.currentTime/audioElement.duration)*100);
    progressBar.value = progress
});

myProgressBar.addEventListener("change", () => {
    audioElement.currentTime = myProgressBar.value * audioElement.duration/100;
});

const makeAllPlays = () => {
    Array.from(document.getElementsByClassName("startPlay")).forEach((element) => {
        element.classList.remove("fa-circle-pause");
        element.classList.add("fa-circle-play");
    });

}
Array.from(document.getElementsByClassName("startPlay")).forEach((element) => {
    element.addEventListener("click", (e) => {
        songIndex = parseInt(e.target.id);
        console.log(songIndex);

        makeAllPlays();
        e.target.classList.remove("fa-circle-play");
        e.target.classList.add("fa-circle-pause");
        audioElement.src = `./songs/${songIndex+1}.mp3`;
        masterSongName.innerText = songs[songIndex].songName;
        audioElement.currentTime = 0;
        audioElement.play();
        changeGif.style.opacity = 1;
        console.log(audioElement.currentTime);
    })
})

document.getElementById('next').addEventListener("click", () => {
    if(songIndex>=9){
        songIndex = 0;
    }
    else{
        songIndex += 1;
    }
    audioElement.src = `./songs/${songIndex+1}.mp3`;
    masterSongName.innerText = songs[songIndex].songName;
    audioElement.currentTime = 0;
    audioElement.play();
    masterPlay.classList.remove('fa-circle-play');
    masterPlay.classList.add('fa-circle-pause');
    changeGif.style.opacity = 1;

})

document.getElementById('previous').addEventListener("click", () => {
    if(songIndex<=0){
        songIndex = 0;
    }
    else{
        songIndex -= 1;
    }
    audioElement.src = `./songs/${songIndex+1}.mp3`;
    masterSongName.innerText = songs[songIndex].songName;
    audioElement.currentTime = 0;
    audioElement.play();
    masterPlay.classList.remove('fa-circle-play');
    masterPlay.classList.add('fa-circle-pause');
    changeGif.style.opacity = 1;

})


// const makeAllPlays = () => {
//     Array.from(document.getElementsByClassName("startPlay")).forEach((element) => {
//         element.classList.remove("fa-circle-pause");
//         element.classList.add("fa-circle-play");
//     });

// }


const linkPlayBottons = () =>{
    makeAllPlays();
    Array.from(document.getElementsByClassName("startPlay")).forEach((element) => {
        console.log(element.classList[3]);
        let linkbtn =element.classList[3];
        if(linkbtn === "fa-circle-play" && audioElement.paused){
            element.classList.remove("fa-circle-play");
            element.classList.add("fa-circle-pause");
            audioElement.pause();
        }
        else{
            element.classList.remove("fa-circle-pause");
            element.classList.add("fa-circle-play");
            audioElement.play();
        }
    })
}
